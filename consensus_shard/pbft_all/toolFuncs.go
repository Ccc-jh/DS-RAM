...
func (p *PbftConsensusNode) GetNeighborNodesReputation() map[uint64]float64 {
	neighborReputationMap := make(map[uint64]float64)
	for nodeID, reputation := range ReputationMap[p.ShardID] {
		if nodeID != p.NodeID {
			neighborReputationMap[nodeID] = reputation
		}
	}
	return neighborReputationMap
}
